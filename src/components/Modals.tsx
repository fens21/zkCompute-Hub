  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
      <div style={{ background: '#111', border: '1px solid #444', padding: 32, borderRadius: 10, maxWidth: 420, width: '90%' }}>
        ...
        <p style={{ opacity: 0.7, fontSize: 12 }}>{job.title} &mdash; {job.reward} {job.tokenSymbol || 'zkLTC'}</p>
        <input placeholder="0xproof..." value={proofHash} onChange={e => onProofHashChange(e.target.value)} style={{ width: '100%', background: '#000', border: '1px solid #444', padding: 12, color: '#fff', margin: '16px 0', fontSize: 13 }} />
        <div style={{ display: 'flex', gap: 12 }}>
...
export function ConfirmModal({ action, onCancel, onConfirm }: { action: ConfirmAction; onCancel: () => void; onConfirm: () => void }) {
  ...
  const msg = action.type === 'unclaim' ? `Are you sure you want to unclaim "${action.job?.title}"?` : action.type === 'deactivate' ? `Deactivate "${action.job?.title}"? No workers have claimed yet.` : action.type === 'dispute' ? `File a dispute for "${action.job?.title}"?\n\nWorker: ${(action.disputeWorker || '').slice(0, 8)}...\nReason: ${action.disputeReason || '—'}` : action.type === 'resolveCancel' ? `Accept cancellation for "${action.job?.title}"? This will remove your claim.` : `Verify and submit proof for "${action.job?.title}"?`
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 200 }}>
      <div style={{ background: '#111', border: '1px solid #444', padding: 32, borderRadius: 12, maxWidth: 420, width: '90%' }}>
        ...
        <p style={{ opacity: 0.8, whiteSpace: 'pre-wrap' }}>{msg}</p>
        <div style={{ display: 'flex', gap: 12, marginTop: 24 }}>
          ...
          <button onClick={onConfirm} style={{ flex: 1, padding: 12, background: btnColor, color: action.type === 'unclaim' || action.type === 'deactivate' ? '#fff' : '#000', fontWeight: 700, border: 'none', borderRadius: 8 }}>{btnLabel}</button>
        </div>
      </div>
    </div>
  )
      ...
      <div style={{ background: '#111', border: '1px solid #444', padding: 32, borderRadius: 12, width: '90%', maxWidth: 440, maxHeight: '90vh', overflow: 'auto' }}>
        ...
        <div style={{ marginBottom: 24, display: 'flex', alignItems: 'center', gap: 16 }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <button onClick={() => fileInputRef.current?.click()} disabled={uploadingAvatar} style={{ padding: '6px 14px', background: '#222', border: '1px solid #555', color: '#fff', borderRadius: 6, fontSize: 11, cursor: 'pointer' }}>
        ...
        <div style={{ display: 'flex', gap: 12, marginTop: 28 }}>
          ...
          <button onClick={onSave} disabled={uploadingAvatar} style={{ flex: 1, padding: 12, background: '#ffd700', color: '#000', fontWeight: 700, border: 'none', borderRadius: 8 }}>{uploadingAvatar ? 'UPLOADING...' : 'SAVE'}</button>
        </div>
      </div>
    </div>
  )
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
      <div style={{ background: '#111', border: '1px solid #f97316', padding: 32, borderRadius: 12, maxWidth: 440, width: '90%' }}>
        ...
        <div style={{ marginBottom: 12 }}>
          <div style={{ fontSize: 12, opacity: 0.7 }}>Job: <strong>{dispute.job.title}</strong></div>
        ...
        <textarea placeholder="Describe your reason for dispute..." value={dispute.reason} onChange={e => onReasonChange(e.target.value)} style={{ width: '100%', background: '#000', border: '1px solid #444', padding: 12, color: '#fff', minHeight: 100, fontSize: 13, borderRadius: 8 }} />
        <div style={{ display: 'flex', gap: 12, marginTop: 20 }}>
          <button onClick={onClose} style={{ flex: 1, padding: 12, border: '1px solid #555', background: 'transparent', color: '#c0c0c0', borderRadius: 8 }}>CANCEL</button>
          <button onClick={onSubmit} disabled={loading || !dispute.reason.trim()} style={{ flex: 1, padding: 12, background: '#f97316', color: '#000', fontWeight: 700, border: 'none', borderRadius: 8 }}>{loading ? 'FILING...' : 'FILE DISPUTE'}</button>
        </div>
      </div>
    </div>
  )
