    ...
    if (remaining <= 0) {
      deadlineDisplay = (
        <span style={{ color: colors.red }}>
          {formatDeadlineDate(job.createdAt, job.deadline)} (expired)
        </span>
      );
    } else {
      ...
      deadlineDisplay = (
        <span style={{ color }}>
          {formatDeadlineDate(job.createdAt, job.deadline)} (
    ...
    >
      ...
      >
        ...
        >
          ...
          <div>
            <strong>Reward:</strong> {job.reward} {job.tokenSymbol || "zkLTC"}
          </div>
          <div>
            <strong>Deadline:</strong> {deadlineDisplay}
          </div>
          <div>
            <strong>Slots:</strong> {job.claimedCount}/{job.maxWorkers} workers
          </div>
          <div>
            <strong>Posted by:</strong> {shorten(job.poster)}
          </div>
          {job.parameters && Object.keys(job.parameters).length > 0 && (
            ...
            >
              ...
              {Object.entries(job.parameters).map(([key, val]) => {
                if (!val) return null;
                const label =
                  JOB_TYPE_CONFIGS[job.type]?.fields.find((f) => f.key === key)
                    ?.label || key;
                return (
                  <div
                    key={key}
          ...
          {job.verificationMethod && (
            <div style={{ marginTop: 8 }}>
              ...
              {job.verificationMethod === "zk-proof"
                ? "On-chain ZK Proof"
                : job.verificationMethod === "hash-check"
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div
          style={{ opacity: 0.7, fontSize: fontSizes.base, marginBottom: 20 }}
        >
          {job.title} &mdash; {job.reward} {job.tokenSymbol || "zkLTC"}
        </div>
          ...
          >
            ...
            {isHashing ? (
              ...
              >
                ...
                />
                <div style={{ fontSize: fontSizes.sm, color: colors.blue }}>
                  Hashing file...
                </div>
              </div>
            ) : fileName && !fileError ? (
              ...
              >
                ...
                <div style={{ fontSize: fontSizes.xs, opacity: 0.4 }}>
                  {fmtSize(fileSize)}
                </div>
                <div
                  role="button"
              ...
              >
                <div style={{ fontSize: fontSizes.sm, color: colors.red }}>
                  {fileError}
                </div>
                <div style={{ fontSize: fontSizes.xs, opacity: 0.5 }}>
              ...
              >
                <div style={{ fontSize: fontSizes.heading }}></div>
                <div style={{ fontSize: fontSizes.base, opacity: 0.7 }}>
                  Drop proof file here
                </div>
                <div style={{ fontSize: fontSizes.xs, opacity: 0.35 }}>
                  or click to browse &bull; any file type &bull; max 10MB
                </div>
              </div>
            )}
          </div>
        ...
        <input
          ...
          value={proofHash}
          onChange={(e) => {
            onProofHashChange(e.target.value);
            if (e.target.value !== proofHash && fileName) clearFile();
          }}
        ...
        <div style={{ display: "flex", gap: 12, marginTop: 20 }}>
          <button
            onClick={onClose}
            disabled={isHashing}
            aria-label="Cancel proof submission"
            style={{
              flex: 1,
              padding: 12,
              border: "1px solid #555",
          ...
          <button
            onClick={onSubmit}
            disabled={loading || isHashing || !proofHash}
            aria-label="Submit proof"
            style={{
              flex: 1,
              padding: 12,
              background: !proofHash
                ? colors.borderLight
                : loading
                ? "#b8960f"
                : colors.gold,
              color: !proofHash ? "#666" : "#000",
              fontWeight: 700,
              border: "none",
              borderRadius: radii.sm,
              cursor:
                loading || !proofHash || isHashing ? "not-allowed" : "pointer",
              transition: "all 0.2s",
            }}
          >
            {loading
...
export function ConfirmModal({
  ...
  const typeLabel =
    action.type === "unclaim"
      ...
      : "Submit Proof?";
  const refundAmount =
    action.type === "deactivate" && action.job
      ? `${action.job.reward * action.job.maxWorkers} ${
          action.job.tokenSymbol || "zkLTC"
        }`
      : "";
  const claimantsInfo =
    action.claimantCount && action.claimantCount > 0
  ...
  const btnLabel =
    action.type === "unclaim"
      ...
      : "VERIFY PROOF";
  const btnColor =
    action.type === "unclaim" || action.type === "deactivate"
      ? colors.red
      : action.type === "resolveCancel"
      ? colors.orange
      : colors.gold;
    ...
    >
      ...
      >
        ...
        <div style={{ marginBottom: 18 }}>
          <div
            style={{ fontSize: fontSizes.sm, opacity: 0.7, marginBottom: 6 }}
          >
            Bio
          </div>
          <textarea
        ...
        <div style={{ marginBottom: 8 }}>
          <div
            style={{ fontSize: fontSizes.sm, opacity: 0.7, marginBottom: 8 }}
          >
            Skills
          </div>
          <div
            ...
            {skills.map((s, i) => (
              ...
              >
                ...
                <span
                  ...
                  style={{ cursor: "pointer", color: "#f66", fontWeight: 700 }}
                >
                  &times;
                </span>
              </span>
            ...
            <input
              ...
              onKeyDown={(e) => {
                if (e.key === "Enter" && editSkillInput.trim()) {
                  setSkills([...skills, editSkillInput.trim()]);
                  setEditSkillInput("");
                }
              }}
    ...
    >
      ...
      >
        ...
        <div style={{ marginBottom: 12 }}>
          <div style={{ fontSize: fontSizes.base, opacity: 0.7 }}>
            Job: <strong>{dispute.job?.title || "Unknown"}</strong>
          </div>
          <div style={{ fontSize: fontSizes.base, opacity: 0.7 }}>
            Reward: {dispute.job?.reward || "—"}{" "}
            {dispute.job?.tokenSymbol || "zkLTC"}
          </div>
          {dispute.worker && (
            <div
              style={{
        ...
        <div style={{ position: "relative" }}>
          <textarea
            placeholder="Describe your reason for dispute... (min 10 characters)"
        ...
        {reasonLen > 0 && reasonLen < 10 && (
          <div style={{ fontSize: 9, color: colors.orange, marginTop: 2 }}>
...
export function ZKSolutionModal({
  ...
  const target =
    job.expectedOutput ||
    job.parameters?.expectedOutput ||
    "— (job poster did not set a target)";
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-label="Submit ZK solution proof"
    >
      <div
        style={{
          background: colors.bgCard,
      ...
      >
        <h3 style={{ marginTop: 0, marginBottom: 4, color: "#c4b5fd" }}>
          Submit ZK Proof (Verifiable Compute)
        </h3>
        <div
          style={{ opacity: 0.75, fontSize: fontSizes.base, marginBottom: 16 }}
        >
        ...
        >
          Enter the <strong>solution / computed result</strong> you found for
          this job. The ZK proof will cryptographically prove that you know the
          correct value that matches the job's target — without revealing your
          solution on-chain.
        </div>

        <div style={{ marginBottom: 8 }}>
          <div
            style={{ fontSize: fontSizes.xs, opacity: 0.5, marginBottom: 4 }}
          >
            JOB TARGET (expectedOutput)
          </div>
          <div
            style={{
              fontFamily: "monospace",
        ...
        <div style={{ marginBottom: 4 }}>
          ...
          <div style={{ fontSize: 10, opacity: 0.4, marginTop: 4 }}>
            This value stays in your browser. Only the ZK proof (that you know
            it) goes on-chain.
          </div>
        </div>
