...
// ── Identicon generator (deterministic from address) ──────────────────────
function generateIdenticon(address: string): string {
  ...
  // Encode sebagai base64 bukan utf8
...
export function Profile({ account, myJobs, bio, skills, avatarUrl, setEditBio, setShowEditProfile, leaderboard, ltcPrice }: {
  account: string
  myJobs: Job[]
  bio: string
  skills: string[]
    ...
    <div style={{ maxWidth: 720, margin: '0 auto' }}>
      <div style={{ background: '#111', padding: 28, border: '1px solid #333', borderRadius: 16 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 24 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ display: 'flex', gap: 8, marginTop: 8, flexWrap: 'wrap' }}>
              <button
                onClick={copyAddress}
                style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '4px 10px', background: copied ? '#1a3c1a' : '#1a1a1a', border: `1px solid ${copied ? '#4ade80' : '#444'}`, color: copied ? '#4ade80' : '#aaa', borderRadius: 6, fontSize: 11, cursor: 'pointer', transition: 'all 0.2s' }}
              >
              ...
              <a
                ...
                style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '4px 10px', background: '#1a1a1a', border: '1px solid #444', color: '#aaa', borderRadius: 6, fontSize: 11, textDecoration: 'none', transition: 'all 0.2s' }}
              >
                🔗 View on Explorer
              </a>
            </div>
          ...
          <button
            ...
            style={{ padding: '6px 14px', background: '#222', border: '1px solid #555', color: '#fff', borderRadius: 6, fontSize: 11, cursor: 'pointer', flexShrink: 0 }}
          >
            Edit Profile
          </button>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(130px, 1fr))', gap: 12, marginBottom: 16 }}>
          ...
          <div style={{ background: '#1a1a1a', padding: '12px', borderRadius: 8, textAlign: 'center' }}>
            ...
            <div style={{ fontSize: 14, fontWeight: 700, marginTop: 4, color: '#4ade80' }}>
              {earnedZkltc + earnedUsdc === 0 ? '—' : ltcPrice && earnedUsd > 0 ? `$${earnedUsd < 1000 ? earnedUsd.toFixed(0) : (earnedUsd / 1000).toFixed(1) + 'k'}` : `${earnedZkltc + earnedUsdc}`}
            </div>
            {earnedZkltc > 0 && <div style={{ fontSize: 11, color: '#ffd700', fontWeight: 600 }}>{earnedZkltc} zkLTC</div>}
            {earnedUsdc  > 0 && <div style={{ fontSize: 11, color: '#2775ca', fontWeight: 600 }}>{earnedUsdc} USDC</div>}
          </div>
        </div>
            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', position: 'relative' }}>
              {badges.map((badge, i) => (
                <div
                  ...
                  style={{ position: 'relative', display: 'flex', alignItems: 'center', gap: 5, padding: '5px 10px', background: '#1a1a1a', border: `1px solid ${badge.color}33`, borderRadius: 999, fontSize: 11, color: badge.color, cursor: 'default', transition: 'background 0.2s' }}
                >
        ...
        <div style={{ marginBottom: 20 }}>
          ...
          {bio ? (
            ...
            <div style={{ fontSize: 11, opacity: 0.35, fontStyle: 'italic' }}>
              No bio yet —{' '}
              <span onClick={() => { setEditBio(''); setShowEditProfile(true) }} style={{ color: '#ffd700', cursor: 'pointer', textDecoration: 'underline' }}>
                add one
              </span>
            </div>
          )}
              ...
              {recentActivity.map((job, i) => (
                <div key={i} style={{ background: '#1a1a1a', padding: '10px 14px', borderRadius: 6, fontSize: 11, display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8 }}>
                  ...
                  <span style={{
                    ...
                    {job.status === 'paid'      ? `+${job.reward} ${job.tokenSymbol || 'zkLTC'}` :
